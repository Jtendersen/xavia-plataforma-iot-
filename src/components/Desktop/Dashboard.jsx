...
function DashboardContent() {
  ...
  const bateria =
    measures[0] && measuresByDay ? (
      Math.ceil(
        measuresByDay[measuresByDay.length - 1]?.DevEUI_uplink?.payload
          .batteryVoltage / 0.038
      ) + "%"
    ) : (
      <></>
    );
    ...
    <>
      ...
      >
        ...
        <Stack direction="row" justifyContent="flex-start" alignItems="center">
          ...
          <Box sx={{ p: 1, fontSize: 22, fontWeight: "bold" }}>
            {measures && measures[deviceArrIndex][0]?.DevEUI_uplink.DevEUI}
          </Box>
          <IconButton onClick={() => handleDeviceClick(1, deviceArrIndex)}>
            <ArrowForwardIosIcon />
          </IconButton>
            ...
            <Grid container spacing={2}>
              ...
              <Grid item xs={12} md={4} lg={4}>
                ...
                >
                  ...
                  <CyclesUsePie
                    measures={measures[0] && getCycles(measuresByDay)}
                  />
                </Paper>
              </Grid>
