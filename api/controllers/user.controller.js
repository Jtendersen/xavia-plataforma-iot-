...
class UserController {
  ...
  static async findByName(req, res) {
    try {
        ...
        return user.length===0?res.status(404).send(errorHandler(0,user)):res.status(200).send(user)
    } catch (error) {
        console.log(error)
    }
}
  ...
  static async userModify(req, res) {
    try {
      ...
      return user.matchedCount==0?res.status(404).send(errorHandler(1,user)):res.status(400).send(errorHandler(2,user))
    } catch (error) {
      console.log(error);
    }
  }
