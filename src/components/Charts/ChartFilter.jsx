  ...
  React.useEffect(() => {
    ...
    (async function fetchDistance() {
      ...
      const { data } = await axios.get(
        `/api/measures/all?entries=${time}&user=${
          loggedUser.roles[0] === "admin" ? user : loggedUser._id
        }&device=${device}`
      );
    ...
    >
      ...
      <FormControl fullWidth>
        ...
        >
          ...
          <MenuItem id="time6" value={9999999999999999}>
            Todo
          </MenuItem>
        </Select>
      </FormControl>
      ...
      <Box>
        Ha recorrido:{" "}
        {Number(
          dataSet.reduce((sum, { distance }) => sum + distance, 0).toFixed(2)
        )}{" "}
        metros
      </Box>
    </Stack>
