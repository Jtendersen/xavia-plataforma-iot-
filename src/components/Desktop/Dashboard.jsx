...
function DashboardContent() {
  ...
  function sumarDias(date, dias){
    ...
    const handleDeviceClick = (num,deviceArrIndex) => {
        if(deviceArrIndex===0&&num===-1){return setDeviceArrIndex(measures.length-1)}
        else if(deviceArrIndex===measures.length-1&&num===1){return setDeviceArrIndex(0)}
        return setDeviceArrIndex(deviceArrIndex + num)
        
    }
  
    
    const measuresByDay = filteredMeasures[deviceArrIndex]&&measures[deviceArrIndex].filter((e)=>{
  ...
  return (
    ...
    <Stack direction="row"
                ...
                alignItems="center">
            <IconButton onClick={()=> handleDeviceClick(-1, deviceArrIndex)}><ArrowBackIosIcon/></IconButton>
            <Box sx={{p:1, fontSize: 22, fontWeight: 'bold'}}>{measures&&measures[deviceArrIndex][0].DevEUI_uplink.DevEUI}</Box>
            <IconButton onClick={()=> handleDeviceClick(1, deviceArrIndex)}><ArrowForwardIosIcon/></IconButton>
        </Stack>
